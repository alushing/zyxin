...
def compute_coloc(folder_tif):
    ...
    for tif_filename in sorted(glob(op.join(folder_tif, '*.tiff'))):
        ...
        ### make masks
        (actn1_zyx, actn1_nozyx, actn1_mask, zyx_mask) = make_masks(img_end)
        ...
        if actn1_zyx_events > 0: 
            actn1_ratio = cal_intR(actn1_zyx_regprops, img_end, actn1_mask, channel=1)
            zyxin_ratio = cal_intR(actn1_zyx_regprops, img_end,  zyx_mask, channel=2)
...
def make_masks(img_yxc):
    ...
    actn1_thresh = actn1_filtered > sf.threshold_yen(actn1_filtered)
